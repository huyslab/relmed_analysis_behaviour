...
begin
	...
	dest_folder = "$(script_dir)/trial1_stimuli/"
	using DataFrames, CSV, Random, Images, ImageTransformations, StatsBase
end
...
begin
	...
	mem_image = DataFrame(CSV.File("$(data_dir)/THINGS_Memorability_Scores.csv"))

	mem_category = DataFrame(CSV.File("$(data_dir)/Concept_to_category_linking.csv"))
end
...
selected_images = let rng = Xoshiro(0)
	...
	selected_images.file_path = (x -> replace(x, "images/" => "$(script_dir)/THINGS/")).(selected_images.file_path)
...
let
	isdir(dest_folder) || mkdir(dest_folder)
	...
	CSV.write("$(dest_folder)/stimuli.csv", selected_images)
...
let
	...
	for file in selected_images.filename
	    img = load(joinpath(dest_folder, file))     # Load image
	    resized_img = imresize(img, (500, 500))  # Resize to 500x500
	    save(joinpath(dest_folder, file), resized_img)  # Save resized image
	end
