...
function quality_checks(
    ...
    if "STUDY_ID" in names(df) 
        ...
        prolific_ids = combine(
            groupby(prolific_ids, [experiment.participant_id_column, :session]),
            :STUDY_ID => (x -> join(x, ",")) => :study_id,
            :SESSION_ID => (x -> join(x, ",")) => :session_id
        )
        leftjoin!(quality, prolific_ids, on=[experiment.participant_id_column, :session])
    end
    ...
    module_start_times = combine(
        ...
        :module_start_time => (x -> [unique(x)]) => :all_module_start_times
    )
