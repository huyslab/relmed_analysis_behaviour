---
title: "Control - Exploration trials"
author: "Haoyang Lu"
format:
  html:
    toc: true
...
## Tidy up data for follow-up analyses
...
## What drives participants' exploration behavior?

### By the number of occurrence of choices
...
### By the length of intervals: working memory
...
### Are these two indices correlated?
...
### By the times of controlled choice: information & uncertainty
...
Although it seems that the times of having control can predict the choices, the statistical test says the no.
...
let
  ...
  # GLMM
  glmm(@formula(response_left ~ session * n_controlled_scaled * interval_scaled + (session * n_controlled_scaled * interval_scaled | prolific_pid)), explore_indices_df, Bernoulli(), contrasts=Dict(:session => EffectsCoding()), fast=true, progress=false)
end
```
