...
function llrwdbda(x::AbstractVector{T},D::Any) where T

	beta1 = exp(x[1]); 
	alfa1 = 1/(1+exp(-x[2])); 
	beta2 = exp(x[1]+x[3]); 
	alfa2 = 1/(1+exp(-(x[2]+x[4]))); 
   ...
   l = 0.0
	...
	for t = 1:Tt
      if t<=Tt/2; alfa=alfa1; beta=beta1; else alfa=alfa2;beta=beta2; end
