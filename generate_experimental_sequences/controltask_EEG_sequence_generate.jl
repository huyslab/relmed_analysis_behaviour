...
# Sample half of the next islands: one trial where the next island is the homebase of the choice ships, and one trial where the next island is not
stimuli_list = []
for wind in 1:3
...
CSV.write("EEG_exploration_sequence.csv", DataFrame(flattened_stimuli_sequence))

open("EEG_exploration_sequence.json", "w") do io
...
for (i, perm) in enumerate(selected_perms)
  ...
  append!(prediction_sequence, [(; ship = ship_name[perm])])
end
CSV.write("EEG_prediction_sequence.csv", DataFrame(prediction_sequence))
open("EEG_prediction_sequence.json", "w") do io
...
# Assign current values so they're balanced across the dataset
attempt = 0
max_attempts = 10000000
while attempt < max_attempts
  # Assign currents randomly within each target_island group
  transform!(groupby(reward_sequence, [:target_island]), :target_island => (x -> sample(repeat(1:3, ceil(Int, length(x) / 3)), length(x), replace=false)) => :current)

  # Check if currents are balanced
  current_counts = combine(groupby(reward_sequence, :current), nrow => :count)
  target_current_counts = combine(groupby(reward_sequence, [:target_island, :current]), nrow => :count)
  if length(unique(current_counts.count)) == 1 && nrow(target_current_counts) == 12
    @info "Found balanced currents after $(attempt) attempts"
    break
  end
...
if attempt == max_attempts
  error("Could not balance currents after $max_attempts attempts")
end
# Add reward amount column (50p for half, £1 for half) within each target_island and current combination
transform!(groupby(reward_sequence, [:current])) do group
  n = nrow(group)
...
# Save to files
CSV.write("EEG_reward_sequence.csv", DataFrame(reward_sequence_tuples))
open("EEG_reward_sequence.json", "w") do io
