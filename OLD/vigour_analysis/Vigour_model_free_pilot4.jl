...
begin
    ...
    Pkg.instantiate()
	using Random, DataFrames, JSON, CSV, StatsBase, JLD2, HTTP, Dates, AlgebraOfGraphics, CairoMakie, Tidier, MixedModels, MixedModels, Printf, CategoricalArrays
...
begin
	CSV.write("data/pilot4_raw_pit_data.csv", raw_pit_data)
	CSV.write("data/pilot4_raw_vigour_data.csv", raw_vigour_data)
end
...
@distinct(vigour_data, prolific_pid, version, age) |>
	x -> combine(groupby(x, [:version]), :age => (x -> DataFrame([summarystats(x)])) => AsTable)
...
plot_presses_vs_var(@filter(vigour_data, trial_number != 1); x_var=:ratio, y_var=:press_per_sec, grp_var=:version, ylab="Press/sec", xlab = "Fixed ratio", combine=false)

# ╔═╡ c4e7d7ec-8da8-4a1f-8da3-0f2d29be6db7
plot_presses_vs_var(@filter(vigour_data, trial_number != 1); x_var=:magnitude, y_var=:press_per_sec, grp_var=:version, ylab="Press/sec", xlab = "Reward magnitude", combine=false)
...
begin
	...
	pred_effect_rpp = crossjoin(DataFrame.((pairs((;prolific_pid="New",chose_left=0.5,diff_rpp=diff_rpp_range,version=["4.0"]))...,))...)
...
plot_presses_vs_var(@mutate(pit_data, coin = coin); x_var=:coin, y_var=:press_per_sec, grp_var=:version, xlab="Pavlovian stimuli (coin)", ylab = "Press/sec", combine=false)
...
let
	...
	p = data(avg_w_data) *
	mapping(:coin, :avg_y, col=:reward_per_press=>nonnumeric) *
	(
