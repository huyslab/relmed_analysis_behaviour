...
# ╔═╡ ce7fde0f-0e62-4238-8e2f-38ed2f019772
...
PLT_data, test_data, vigour_data, post_vigour_test_data, pit_data, jspsych_data = load_pilot5_data(force_download=true);
...
# ╔═╡ b5eb4a75-c965-44a5-b4e0-26db60c80899
let
	sub_list = CSV.read("data/prolific_participant_info/prolific_export_6723b4962fef7ca334bb043b.csv", DataFrame)
	...
	# select!(finished_df, :prolific_pid)
	for row in eachrow(finished_df)
		println(join(row, ","))
	end
end

# ╔═╡ 6868f8e9-9911-4c90-b0d2-ad7681be748c
summarize_participation(jspsych_data)

# ╔═╡ ecd4060f-21a1-4c98-93c6-c33211c7a485
summarize_participation(jspsych_data) |>
	filter(x -> x.version in ["5.0"] && !ismissing(x.finished))
