...
begin
	...
	sess1 = filter(x -> x.block <= 10, pilot6_wm)
	nothing
end
...
let
	...
	# Define parameters
	ρ1, a1s, ll1 = qs_ests.ρ, qs_ests.a1, qs_ests.loglike
	ρ2, a1r, ll2 = qr_ests.ρ, qr_ests.a1, qr_ests.loglike
	...
	scatter!(ax1, ρ1, a1s, ll1)
	scatter!(ax2, ρ2, a1r, ll2)
	f
end
...
let
	choice_df1, choice_df2 = true_vs_preds(qs_choices, qr_choices; strct = sess1)
...
let
	choice_df1, choice_df2 = true_vs_preds(wm_choices, wmr_choices; strct = sess1)
	plot_true_vs_preds(choice_df1, choice_df2)
end
...
#=╠═╡
let
	choice_df1, choice_df2 = true_vs_preds(wmss_choices, wmssr_choices; strct = sess1)
	plot_true_vs_preds(choice_df1, choice_df2)
end
  ╠═╡ =#
...
# ╔═╡ bfeb3a32-87d4-45a1-b1c9-c2c522366a73
md"
All even-odd splits based on trial index (block length = 7, so this makes sense.)
"

# ╔═╡ 9d4946da-6ad0-4fbc-909b-56b58a79eb5e
md"
#### Model-free internal consistency
"

# ╔═╡ e54a66a6-762b-42fd-9c68-b4680649f0f0
let
	mean_acc = combine(
	...
	workshop_reliability_scatter!(
		...
		xcol=Symbol("true"),
		ycol=Symbol("false"),
		subtitle="Session 1 split-half: overall mean accuracy",
		correct_r=false
	)
	workshop_reliability_scatter!(
		fig[2,1];
		...
		xcol=Symbol("true"),
		ycol=Symbol("false"),
		subtitle="Session 1 split-half: mean accuracy (set size = 3)",
		correct_r=false
	)
	workshop_reliability_scatter!(
		fig[2,2];
		...
		xcol=Symbol("true"),
		ycol=Symbol("false"),
		subtitle="Session 1 split-half: mean accuracy (set size = 21)",
		correct_r=false
	)
	workshop_reliability_scatter!(
		fig[3,1:2];
		...
		xcol=Symbol("true"),
		ycol=Symbol("false"),
		subtitle="Session 1 split-half: mean difference in accuracy",
		correct_r=false
	)
	fig
end
...
let
	...
	workshop_reliability_scatter!(
		...
		xcol=:ρ,
		ycol=:ρ_1,
		subtitle="Session 1 split-half: reward sensitivity",
		correct_r=false
	)
	workshop_reliability_scatter!(
		fig[2,1];
		...
		xcol=:a1,
		ycol=:a1_1,
		subtitle="Session 1 split-half: learning rate (set-size = 3)",
		correct_r=false
	)
	workshop_reliability_scatter!(
		fig[2,2];
		...
		xcol=:a2,
		ycol=:a2_1,
		subtitle="Session 1 split-half: learning rate (set-size = 21)",
		correct_r=false
	)
	workshop_reliability_scatter!(
		fig[3,1:2];
		...
		xcol=:Δa_s1,
		ycol=:Δa_s2,
		subtitle="Session 1 split-half: learning rate difference",
		correct_r=false
	)
	fig
end
