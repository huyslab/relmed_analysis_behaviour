...
for k=1:4; T[:,k] = Tci[:,k,:]*w[end,:]; end

p1=plot(c[1,:],ls=:dash,lw=[1],lc=[ :blue])
scatter!(c[1,:],mc=:blue)
plot!(cinf',ls=:solid,lw=[2],lc=[ :red])
