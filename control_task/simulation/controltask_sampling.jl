...
function infer()
   ...
   bx = 0.1 * betatrue' .+ 10 .+ rand(Normal(0, 1), nbeta, nSamples)
   ...
   for t = 1:nTrials
      ...
      for k = 1:nSamples
         ...
         # bxn += rand(Normal(0, sigma), nbeta)

         wxn = rand(Dirichlet(wx[:, k] * alpha), 1)
         wxn = wxn .+ 1e-50
         wxn = wxn ./ sum(wxn) # avoid zeros 
...
function myplots()
   ...
   local p1 = plot(c[1, :], ls=:dash, lw=1, lc=:blue)
   scatter!(c[1, :], mc=:blue)
   plot!(pc[:, 1], ls=:solid, lw=1, lc=:red, legend=false)
   title!("control inference", titlefontsize=fs)
   xlabel!("Trial")

   # fraction of control events (left) and no control (right) correction
   # identified 
   local p3 = bar([mean(pc[findall(c[1, :] .== 1), 1]); mean(pc[findall(c[1, :] .== 0), 2])], legend=false)
   ylims!(0, 1)
   xticks!([1, 2], ["c=1", "c=0"])
   ...
   local p2 = plot(resampled .* maximum(betahat), lc=RGB(0.7, 0.7, 0.7))
   ...
   local p5 = plot(xx, [t1 y1 t2 y2 t3 y3], lc=[:red :red :green :green :cyan :cyan], ls=[:solid :dash :solid :dash :solid :dash])
   xlabel!("Effort")
   ylabel!("p(control)")
