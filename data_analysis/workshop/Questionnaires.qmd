...
# # WM
# wm <- read_csv(here("results", "workshop", "wm.csv"))
...
```{r}
full_df <- reduce(list(pilt, vigour, pit, reversal, generalization, qn), full_join) %>% 
...
```{r}
q_q_corr_plot <- function(df, .session, only_total = F) {
  selected_df <- df %>% select(ends_with(paste0("s", .session)))
  if (only_total) {
      selected_df <- select(selected_df, contains("_total"))
  }
  corr_p_recipe <- correlation(selected_df) %>% 
...
q_q_corr_plot(score_df, 1, only_total = T)
```
...
q_q_corr_plot(score_df, 2, only_total = T)
```
:::
...
task_q_corr_plot <- function(df,
    ...
    filtered_df <- df2 %>%
        ...
        ungroup() %>%
        select(-ends_with("_n", ignore.case = F))
    
    if (is.null(.task)) {
        task_df <- select(filtered_df,
                          matches("^(vigour|pit|pilt|reversal|wm|generalization)"))
    } else {
        task_df <- select(filtered_df, starts_with(.task))
    }
    
    
    if (only_total) {
        q_df <- select(filtered_df, ends_with("total"))
    } else {
        q_df <- select(
            filtered_df,
            -session,
    ...
    corr_p <- corr_p_recipe %>%
        plot() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
        labs(caption = glue::glue("# Obs: [{n_obs[1]}, {n_obs[2]}]")) +
    print(corr_p)
    invisible(corr_df)
}
```
...
task_q_corr_plot(full_df, .session = 1, only_total = T, fail_cutoff = 2, p_adjust = "none") %>% 
    display() # or use display() for printint out correlation data frame
```
...
task_q_corr_plot(full_df, .session = 2, only_total = T, fail_cutoff = 2, p_adjust = "none") %>% 
    display()
```
...
#| fig-height: 8
#| fig-dpi: 300
task_q_corr_plot(full_df, .session = NULL, only_total = T, fail_cutoff = 2, p_adjust = "none") %>% 
    display()
```
...
```{r}
task_task_corr_plot <- function(df, .session, ...) {
    if (is.null(.session)) {
        df2 <- df %>%
            # semi_join(two_session_sub) %>% 
            group_by(prolific_pid) %>%
...
task_task_corr_plot(full_df, .session = 1) %>% 
...
task_task_corr_plot(full_df, .session = 2) %>% 
    invisible() # or use display() for printint out correlation data frame
```
...
task_task_corr_plot(full_df, .session = NULL) %>% 
    invisible() # or use display() for printint out correlation data frame
```
:::
